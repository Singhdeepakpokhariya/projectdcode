...
function Form1() {
  ...
  if(responseInfo.isLoading) {document.getElementById("message").innerHTML = "loading..."}
  if(responseInfo.isError) {document.getElementById("message").innerHTML = "Invalid Credentials Check again"}
  ...
  // if(responseInfo.isError) return <h1  >An error occured{responseInfo.error.error}</h1>
    ...
    // })}
     >
            ...
            <div><p className="text-center pt-1 option Individuals" type="text" placeholder="As Individuals">
              As Individuals
            </p>
            <p className="text-center pt-1   option  School " type="text" placeholder="As Schools">
              
              <Link  className='text-decoration-none ' style={{color:"grey"}}  to="/Student">As Schools</Link>
            </p></div>
           <label> < input
              className="inputarea"
              name='Phone Number'
           ...
           <label> < input
              ...
              // component="input"
              onChange={(password) => passwordHandler(password, SetPassword, SetPasswordErr)}
            ...
            // onClick={gotodashboard} 
             className='mt-3 login-btn' type="button" value="Login" /> 
            <hr className="formhr" />
